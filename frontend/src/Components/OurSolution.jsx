...
const OurSolution = () => {
  ...
  useEffect(() => {
    const circleAnimation = () => {
      ...
      circleWrapper.style.background = `conic-gradient(var(--blue) ${
        percentage + 15
      }%, #1A1A1A 0%)`;
    };
  ...
  useEffect(() => {
    const updateProgressLine = () => {
      ...
      progress = Math.min(progress * 1.8, 1); // Accelerate fill speed

      const minHeight = 5; // Show 50% green line by default
  ...
  return (
    <div className="Our-Solution-Process-section bg-(--darkblack) text-(--whitelight) relative mb-[00px] px-5">
        ...
        <div className="row py-14 lg:py-28">
          <div className="grid lg:grid-cols-3 grid-cols-1 gap-10">
            ...
            <p className="lg:px-2 px-1 text-(--gray) flex items-end justify-start font-bold">
              Success doesn’t happen by chance, it follows a well-planned
              process! At Atrix IT Solutions, we follow a strategic approach to
              ensure smooth execution. Here are the key steps we take to drive
          ...
          <div className="col-1 grid lg:grid-cols-2 grid-cols-1 items-start relative mt-32">
            <div
              id="circle-sec"
              className="lg:flex justify-center sticky lg:top-28 hidden"
            >
              <div
                id="circle-wrapper"
                className="circle-wrapper h-[340px] w-[340px] rounded-[50%] border-[4px] border-[#2F2F2F] flex justify-center items-center"
              >
            ...
            <div
              id="process-cards"
              className="col-2 lg:border-l border-(--black) relative"
            >
              {/* Line wrapper and animated progress */}
              <div
                id="line-progress"
                ...
                style={{
                  ...
                  boxShadow: "0 0 10px 3px rgba(0, 166, 87, 0.4)", // subtle glow
                }}
              />
              ...
              <div className="w-full lg:pl-26 z-50">
                {blurbContent.map((item, index) => (
                  <div
                    key={index}
                    className="blurb-item pb-20 bg-(--darkblack) pl-1 pt-1 sticky top-20"
                  >
                    <img
                      src={item.image}
                      alt={item.title}
                    ...
                    />
                    <p className="lg:hidden inline-block mb-5 bg-(--black) px-5 py-2 rounded-lg font-bold text-sm">
                      STEP {item.id}
                    </p>
                    ...
                    <ul>
                      {item.additionalText.map((text, idx) => (
                        <li
                          className="flex gap-2 items-center text-(--gray) pt-2"
                          key={idx}
                        >
                          <FiPlus /> {text}
                        </li>
