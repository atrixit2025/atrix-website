...
const FaqsCard = ({ faqsList, isActive, onClick }) => {
  ...
  return (
    <div className={`accordian overflow-hidden ${isActive ? "active" : ""}`}>
      <h4
...
const FaqSection = () => {
  ...
  const handleFaqClick = (categoryIndex, faqIndex) => {
    setActiveFaqs((prevActiveFaqs) => {
      const currentActive = prevActiveFaqs[categoryIndex];
      return {
        ...prevActiveFaqs,
        [categoryIndex]: currentActive === faqIndex ? null : faqIndex,
      };
    });
  };
  ...
  return (
    <section className="faqsec py-16 mt-16  ">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {faqsData.map((category, categoryIndex) => (
            <div key={categoryIndex} className="leading-relaxed">
              ...
              <div className="mt-8">
                {category.content.map((faq, faqIndex) => {
                  const isActive = activeFaqs[categoryIndex] === faqIndex;
                  return (
                    <FaqsCard
                      key={`${categoryIndex}-${faqIndex}`}
                      faqsList={faq}
                      isActive={isActive}
                      onClick={() => handleFaqClick(categoryIndex, faqIndex)}
                    />
                  );
                })}
              </div>
