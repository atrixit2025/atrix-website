...
export default function Portfolio() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const portfolios = response.data.Portfolio;

        // Map the fetched data to the tableData format
        const mappedData = portfolios.map((tech, index) => ({
          id: tech._id,
          name: tech.title,
  ...
  const handleDelete = async (id, title, category) => {
    try {
      ...
      const response = await axios.delete("http://localhost:5300/Portfolio/delete", {
        data: { title, category },
      });

      if (response.status === 200) {
        // Remove the deleted Portfolio from the tableData state
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Portfolio"
        buttonText="New Add Portfolio"
        buttonLink="/AddNewPortfolio" />
